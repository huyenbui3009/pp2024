...
def student_input(students):
    ...
    destination= os.path.join(new_directory, "student.txt")
    os.rename("students.txt", destination) #move student.txt into the directory
...
def course_input(courses):
    ...
    new_directory= r"C:\Users\buing\Downloads\pp2024\pw5\result"
    os.makedirs(new_directory, exist_ok=True)
    destination= os.path.join(new_directory, "courses.txt")
    os.rename("courses.txt", destination) 
...
def mark_input(students, courses):
    ...
    if courseID not in courses.keys():
        ...
        filename = r"C:\Users\buing\Downloads\pp2024\pw5\result\marks.txt"
...
def decompress_data(input_file, output_dir):
